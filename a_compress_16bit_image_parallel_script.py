...
from utils.cmd_root_path import cmd_root_path
# change
from utils.sub_work import setup_logging

from utils.Data_Prepare import ImageData
from Config.image_16bit_config import lossless_tuple_codes_high_dynamic_range, tuple_codes_high_dynamic_range

from Config.config_compress import args_16bit_image_compress_config
...
CONNECTION = None
args = args_16bit_image_compress_config()
...
def bisection(inverse, a, b, ab_tol, metric, target, target_tol, codec, image, width, height, subsampling):
    ...
    if args.lossless == False:
        ...
        while (b - a) > ab_tol:
            ...
            quality, encoded_file = f_image_lossy_16bit(LOGGER, c, codec, image, width, height, temp_folder,
                                                        subsampling)
            last_c = c
        ...
        last_c = '0'
        quality, encoded_file = f_image_lossless_16bit(LOGGER, image, width, height, temp_folder, codec, subsampling,
                                                       last_c)
...
def main():
    ...
    if args.lossless == False:
        ...
        show_image_lossy_result(results, only_perform_missing_encodes, LOGGER, TOTAL_ERRORS, TOTAL_METRIC,
                                TOTAL_BYTES,
        ...
        func(data, pool, TUPLE_CODECS, only_perform_missing_encodes, results, metric, target, target_tol)
        show_image_lossless_result(results, only_perform_missing_encodes, LOGGER,
                                   TOTAL_ERRORS, TOTAL_METRIC, TOTAL_BYTES,
