...
from utils.cmd_root_path import cmd_root_path, tuple_codes_high_dynamic_range
...
from Config.config_compress import args_lossy_compress_high_dynamic_range_config, show_and_recode_compress_args
...
def f(param, codec, image, width, height, temp_folder, subsampling):
    ...
    if codec in ['jpeg', 'jpeg-mse', 'jpeg-ms-ssim', 'jpeg-im', 'jpeg-hvs-psnr'] and subsampling in ['420', '444']:
        ...
        ffmpeg_use = True
        if ffmpeg_use == True:
            cmd = ['ffmpeg', '-y', '-i', source_ppm, '-pix_fmt', get_pixel_format_for_encoding_16bit(subsampling),
                   source_yuv]
            run_program(LOGGER, cmd)
    ...
    # 3 OPENJPEG
        ...
        if use_convert == 1:
            ...
            cmd = ['ffmpeg', '-y', '-i', image, '-pix_fmt', get_pixel_format_for_encoding_16bit(subsampling),
                   source_yuv]
            run_program(LOGGER, cmd)
