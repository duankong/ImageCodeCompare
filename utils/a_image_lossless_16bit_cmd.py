...
def f_image_lossless_16bit(LOGGER, image, width, height, temp_folder, codec, subsampling, param_lossless):
    """
    ...
    LOGGER.info("Encoding image [ {} ] with codec [ {} ] in Param [ {} ] LOSSLESS".format(filename,str(codec).upper(),param_lossless))
    ...
    try:
        ...
        LOGGER.error('{}'.format('-'*100))
        LOGGER.error("\033[6;30;42m Encoding image [ {} ] with codec [ {} ] in Param [ {} ] LOSSLESS \033[0m".format(filename,str(codec).upper(),param_lossless))
        LOGGER.error('{}'.format('-'*100))
    else:
