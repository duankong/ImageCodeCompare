...
def f_image_lossy_16bit(LOGGER, image, width, height, temp_folder, codec, subsampling, param):
    ...
    LOGGER.info("Encoding image [ {} ] with codec [ {} ] in Param [ {} ] LOSSY".format(filename,str(codec).upper(),param))
    ...
    try:
        ...
        LOGGER.error("\033[6;30;42m Encoding image [ {} ] with codec [ {} ] in Param [ {} ] LOSSY \033[0m".format(filename, str(
            codec).upper(), param))
        LOGGER.error('{}'.format('-' * 100))
    else:
